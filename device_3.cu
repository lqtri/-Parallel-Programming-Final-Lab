...
__global__ void energyCalculatorKernel(uint8_t *inPixels, int width, int height, int* energy) {
  extern __shared__ uint8_t s_inPixels[];
  int filterWidth = 3;
  int sm_width = blockDim.x + filterWidth - 1;
  int sm_height = blockDim.y + filterWidth - 1;
  float sm_size = sm_width * sm_height;
  int sm_first_row = blockIdx.y * blockDim.y - filterWidth / 2;
  int sm_first_col = blockIdx.x * blockDim.x - filterWidth / 2;

  int n_pixelsInEachThread = ceil(sm_size / (blockDim.x * blockDim.y));
  int thread_idx = threadIdx.y * blockDim.x + threadIdx.x;

  for (int i = 0; i < n_pixelsInEachThread; i++) {
    int sm_element_idx = thread_idx * n_pixelsInEachThread + i;
    if (sm_element_idx >= sm_size)
      break;

    int r = sm_element_idx / sm_width + sm_first_row;
    int c = sm_element_idx % sm_width + sm_first_col;

    if (r < 0) r = 0;
    else if (r > height - 1) r = height - 1;

    if (c < 0) c = 0;
    else if (c > width - 1) c = width - 1;

    s_inPixels[sm_element_idx] = inPixels[r * width + c];
  }
  __syncthreads();
  ...
  if (r < height && c < width) {
    ...
    for (int r_sobel = 0; r_sobel < 3; r_sobel++) {
      for (int c_sobel = 0; c_sobel < 3; c_sobel++) {
        ...
        int i_sm = (threadIdx.y + r_sobel) * sm_width + threadIdx.x + c_sobel;
        xvalue += dc_xSobel[i_sobel] * s_inPixels[i_sm];
		yvalue += dc_ySobel[i_sobel] * s_inPixels[i_sm];
      } 
    }
...
void removeSeamFromDevice (uint8_t* d_inPixels, int * d_energy, int width, int height, int min_index, int8_t * backtrack, dim3 blockSize) {
	...
	for (int h = 0; h < height; h++) {
		...
		dim3 gridSize2((n-min_index)*3/blockSize.x+1,1);
...
void seamCarving(uint8_t * inPixels, int width, int height, uint8_t * grayPixels, int * energy, dim3 blockSize=dim3(1), float scalingPercentage=0.8)
{
	GpuTimer timer;
	timer.Start();
    ...
    int filterWidth = 3;
    size_t smem_size = (blockSize.x + filterWidth - 1) * (blockSize.y + filterWidth - 1) * sizeof(uint8_t);
	energyCalculatorKernel<<<gridSize, blockSize, smem_size>>>(d_grayPixels, width, height, d_energy);
	cudaDeviceSynchronize();
	CHECK(cudaGetLastError());
	...
	for (int w = width; w > new_width; w--){
		...
		findMinsKernel<<<gridSize3, blockSize>>>(d_last_row, w, d_mins, d_min_indices);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());
...
int main(int argc, char ** argv)
{
    ...
    CHECK(cudaMemcpyToSymbol(dc_xSobel, xSobel, 9 * sizeof(float)));
    CHECK(cudaMemcpyToSymbol(dc_ySobel, ySobel, 9 * sizeof(float)));
	...
	seamCarving(inPixels, width, height, grayPixels, energy, blockSize, scalingPercentage);
